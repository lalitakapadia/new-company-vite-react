      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {data.map((item, index) => (
          <motion.div
            ...
            className="relative text-center p-6 rounded-lg shadow-lg bg-gray-700 transition-all duration-500"
          >
            {/* Gradient Background Effect */}
            <motion.div
              className="absolute inset-0 rounded-lg opacity-0 transition-opacity duration-1000"
              whileHover={{ opacity: 1, background: "linear-gradient(135deg, #fff1be, #4fd1c5, #4a90e2, #5e60ff)" }}
            />

            {/* Content */}
            <div className="relative z-10">
              <item.icon className="text-5xl text-blue-300 mb-4 mx-auto transition-all duration-500 group-hover:text-white" />
